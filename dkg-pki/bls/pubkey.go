...
func (pub Pubkey) PublicKey() (pk *blscgo.PublicKey) {
	...
	err := pk.SetHexString(pub.String())
	if err != nil {
...
//	fmt.Printf("proc pk:%s",pk.GetHexString())
	pub.value = []byte(pk.GetHexString())
	return
}
...
//	fmt.Printf("aggregatePubkeys:%s\n",sum.GetHexString())
	pub.value = []byte(sum.GetHexString())
	return
}
