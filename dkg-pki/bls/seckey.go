...
func AggregateSeckeys_ex(secs []Seckey) (sec Seckey) {
	secAggCalls++
	secAggLen += len(secs)

	// convert to blscgo master
	mpk := make([]blscgo.SecretKey, len(secs))
	for i, p := range secs {
		mpk[i] = *(p.SecretKey())
	}
	// derive gshare
	...
	//	fmt.Printf("aggregateSeckeys:%s\n",sk.GetHexString())
	// convert back from blscgo
//	fmt.Printf("aggregateSeckeys:%s\n",sk.GetHexString())
	n :=new(big.Int)
	n.SetString(sk.GetHexString(),16)
	sec.secret = n
	return
...
func ShareSeckey_ex(msec []Seckey, id ID) (sec Seckey) {
	pubShareCalls++
	pubShareLen += len(msec)

	// convert to blscgo master
	mpk := make([]blscgo.SecretKey, len(msec))
	for i, p := range msec {
		mpk[i] = *(p.SecretKey())
	}
	...
	// convert back from blscgo
	i :=new(big.Int)
	i.SetString(sk.GetHexString(),16)
	sec.secret = i
	return
}
...
//	return ShareSeckey(msec, IDFromAddress(addr))
	return ShareSeckey_ex(msec, IDFromAddress(addr))
}
...
//	return RecoverSeckey(secs, ids)
	return RecoverSeckey_ex(secs, ids)
}

func RecoverSeckey_ex(secs []Seckey, ids []ID) (sec Seckey) {
	secRecoverCalls++
	secRecoverLen += len(secs)
	...
	var sk blscgo.SecretKey
	sk.Recover(mpk, mid)
	//	fmt.Printf("recoverSk:%s\n",sk.GetHexString())
	// convert back from blscgo
	p :=new(big.Int)
	p.SetString(sk.GetHexString(),16)
	sec.secret = p
	return
