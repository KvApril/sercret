...
//	p.sharesCombined[addr] = bls.AggregateSeckeys(vlist)
	p.sharesCombined[addr] = bls.AggregateSeckeys_ex(vlist)
	fmt.Printf("secretAggregate:%s\n",p.sharesCombined[addr].Hex())
	return
}
...
//	fmt.Println("msec/vvec start \n")
	for i := 0; i < k; i++ {
		msec[i] = bls.SeckeyFromRand(gseed.Deri(i))
		vvec[i] = bls.PubkeyFromSeckey(msec[i])
	}
	fmt.Printf("proc%d: (addr) %x\n",procNum+1,vvec[0].Address())
	fmt.Printf("proc%d: (sec) %s\n",procNum+1,msec[0].Hex())
	fmt.Printf("proc%d: (pk) %s\n",procNum+1,vvec[0].String())
//	fmt.Println("msec/vvec end \n")
	shares := bls.SeckeyMap{}
	fmt.Printf("\nbase ids(addrs) share proc%d sec  \n",procNum+1)
	procNum +=1
	var addNum int
	addNum = 1
	for _, m := range g.Members() {
		shares[m] = bls.ShareSeckeyByAddr(msec, m)
		fmt.Printf("id%d(addr%d):%x\n",addNum,addNum,m)
		fmt.Printf("secret%d:%s\n\n",addNum,shares[m].Hex())
		addNum +=1
	}
//	fmt.Println("id(addr)/secret end \n")
	return shares, vvec
}
...
//	return fmt.Sprintf("Proc: (sec)%s (seed)%x %s", p.sec.String()[:4], p.rseed.String()[:2], p.reginfo.String())
	return fmt.Sprintf("Proc: (sec)%s (seed)%x %s", p.sec.String()[:], p.rseed.String()[:], p.reginfo.String())
}
