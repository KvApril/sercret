...
func NewGroupSimulator(members []*ProcessSimulator, k uint16) GroupSimulator {
	...
	//	secDup := bls.AggregateSeckeys(secs)
		secDup := bls.AggregateSeckeys_ex(secs)
/*		fmt.Printf("sec:%s\n",sec.String())
		fmt.Printf("secDup:%s\n",secDup.String())
		fmt.Printf("pub:%s\n",pub.String())
		fmt.Printf("pubDup:%s\n",pubDup.String())*/
...
func (g GroupSimulator) Sign(msg []byte) bls.Signature {
	...
	for _, p := range g.proclist {
		...
		fmt.Printf("proc%d: (aggregateSecret) %s\n",procNum2+1,p.sharesCombined[g.Address()].Hex())
		fmt.Printf("proc%d: (sign) %s\n\n",procNum2+1,sigmap[p.Address()].String())
		procNum2 +=1
	}
//	fmt.Println("signforgroup end\n")
	...
	if DoubleCheck {
		...
		fmt.Printf("groupSec:%s\n",g.sec.Hex())
		fmt.Printf("groupSign:%s\n",sig2.String())
//		fmt.Println("group sec end \n")
		//fmt.Printf("sig1.String():%s\n",sig1.String())
		//fmt.Printf("sig2.String():%s\n",sig2.String())
...
//	return fmt.Sprintf("GrpP: (sec)%s %s", g.sec.String()[:4], g.reginfo.String())
	return fmt.Sprintf("\nGrpP: (sec)%s %s", g.sec.Hex()[:], g.reginfo.String())
}
