...
func testPre(t *testing.T) {
	t.Log("init")
	{
		...
		fmt.Printf("same id\n%s\n%s\n", id.GetHexString(), id2.GetHexString())
		...
		fmt.Printf("same id\n%s\n%s\n", id.GetDecString(), id2.GetDecString())
	}
	{
		var sec SecretKey
...
func testAdd(t *testing.T) {
	...
	t.Log("pub1    :", pub1.GetHexString())
	sec1.Add(&sec2)
	t.Log("sec1    :", sec1.GetHexString())
	pub3 := sec1.GetPublicKey()
	t.Log("pub3    :", pub3.GetHexString())
	sign3 := sec1.Sign(m)
	t.Log("sign3    :", sign3.GetHexString())
	if !sign3.Verify(pub3, m) {
		t.Fail()
	}
}
